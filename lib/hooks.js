...
exports.wrapCommand_ = function(cmd, context) {
    return function(callback) {
        ...
        logger.log(logger.clc.cyanBright('[hook]'), truncate(raw.trim(), 120));

        try {
            exec.execSync(raw, {cwd: process.cwd()});
        } catch(e) {
            logger.debug('[hook failure]');
        } finally {
            logger.log(logger.clc.cyanBright('[end of hook]'));
        }
