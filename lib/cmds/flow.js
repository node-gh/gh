...
Flow.prototype[states.AUTO] = function(options) {
    ...
    {
        logger.log('Incorrect syntax: gh flow --auto remote [remote-repo-URL] origin [origin-repo-URL] folder [folder-name]');
    }
    else {
        ...
        async.series(operations, function(error, results) {
            console.log('folder created: '+results[0]);
            console.log('git init: '+results[1]);
            console.log('git remote add origin '+forkedRepoURL);
            console.log('git remote add upstream '+originRepoURL);
