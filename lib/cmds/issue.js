...
Issue.prototype.run = function() {
    ...
    if (options.close) {
        hooks.invoke('issue.close', instance, function(afterHooksCallback) {
            ...
            logger.logTemplate(
                '[info] Closing issue {{greenBright "#" options.number}} on {{greenBright options.user "/" options.repo}}', {
                    options: options
                });
            ...
            instance.close(function(err) {
                ...
                logger.compileTemplate('{{issueLink}}', {
                    options: options
                });
    ...
    if (options.list) {
        if (options.all) {
            logger.logTemplate(
                '[info] Listing {{greenBright options.state}} issues for {{greenBright options.user}}', {
                    options: options
                });
            ...
            logger.logTemplate(
                '[info] Listing {{greenBright options.state}} issues on {{greenBright options.user "/" options.repo}}', {
                    options: options
                });
    ...
    if (options.new) {
        hooks.invoke('issue.new', instance, function(afterHooksCallback) {
            logger.logTemplate(
                '[info] Creating a new issue on {{greenBright options.user "/" options.repo}}', {
                    options: options
                });
    ...
    if (options.open) {
        hooks.invoke('issue.open', instance, function(afterHooksCallback) {
            logger.logTemplate(
                '[info] Opening issue {{greenBright "#" options.number}} on {{greenBright options.user "/" options.repo}}', {
                    options: options
                });
