...
Issue.prototype.list = function(repo, opt_callback) {
    ...
    instance._fetchRepoIssues(payload, function(err, issues) {
        if (err && !options.all) {
...
Issue.prototype._fetchRepoIssues = function(payload, callback) {
    var instance = this;

    base.github.issues.repoIssues(payload, function(err, issues) {
        if (err && /issues.*disabled/i.test(err.message)) {
            // Must be a fork
            base.github.repos.get({user: payload.user, repo: payload.repo}, function(err, repository) {
                if (!err && repository.fork) {
                    var parentRepoPayload = { // TODO: a clone function could be handy
                        user: repository.parent.owner.login,
                        repo: payload.repo,
                        state: payload.state,
                        labels: payload.labels
                    }
                    instance._fetchRepoIssues(parentRepoPayload, callback);
                } else {
                    callback(err, issues);
                }
            });
