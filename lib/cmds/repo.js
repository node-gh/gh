...
Repo.prototype.run = function() {
    ...
    if (options.fork) {
        hooks.invoke('repo.fork', instance, function(afterHooksCallback) {
            ...
            logger.logTemplate('{{prefix}} [info] Forking a repo on {{greenBright user "/" options.repo}}', {
                options: options,
                user: user
            });
            ...
            instance.fork(function(err1) {
                logger.defaultCallback(
                    err1, null, logger.compileTemplate('https://github.com/' + user + '/' + options.repo, { options: options }));
