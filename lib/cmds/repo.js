...
Repo.prototype.run = function() {
    ...
    if (options.fork) {
        hooks.invoke('repo.fork', instance, function(afterHooksCallback) {
            var login = (options.organization) ? options.organization : options.loggedUser;
            options.repo = options.fork;

            logger.logTemplate('{{prefix}} [info] Forking a repo on {{greenBright login "/" options.repo}}', {
                options: options,
                login: login
            });
            ...
            instance.fork(function(err1) {
                logger.defaultCallback(
                    err1, null, logger.compileTemplate('https://github.com/' + login + '/' + options.repo, { options: options }));
