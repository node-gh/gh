...
Gists.prototype.run = function() {
    ...
    if (options.delete) {
        hooks.invoke('gists.delete', instance, function(afterHooksCallback) {
            logger.logTemplate(
                '[info] Deleting gist {{greenBright options.loggedUser "/" options.delete}}', {
                    options: options
                });
    ...
    if (options.fork) {
        hooks.invoke('gists.fork', instance, function(afterHooksCallback) {
            logger.logTemplate(
                '[info] Forking gist on {{greenBright options.loggedUser}}', {
                    options: options
                });

            instance.fork(options.fork, function(err) {
                if (err) {
                    logger.error('Can\'t fork gist.');
                    return;
                }

                logger.compileTemplate(
    ...
    if (options.list) {
        logger.logTemplate(
            '[info] Listing gists for {{greenBright options.user}}', {
                options: options
            });
    ...
    if (options.new) {
        hooks.invoke('gists.new', instance, function(afterHooksCallback) {
            ...
            instance.new(options.new, options.content, function(err, gist) {
                ...
                if (err) {
                    logger.error('Can\'t create gist.');
                    return;
                }

                logger.compileTemplate('{{gistLink}}', {
                    options: options
                });
