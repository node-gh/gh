...
Gists.prototype.run = function() {
    ...
    if (options.fork) {
        hooks.invoke('gists.fork', instance, function(afterHooksCallback) {
            logger.logTemplate('{{prefix}} [info] Forking gist on {{greenBright options.loggedUser}}', {
                options: options
            });
            ...
            instance.fork(options.fork, function(err) {
                logger.defaultCallback(
                    err, null, logger.compileTemplate(
                        'https://gist.github.com/{{options.loggedUser}}/{{options.fork}}', { options: options }));
    ...
    if (options.new) {
        hooks.invoke('gists.new', instance, function(afterHooksCallback) {
            ...
            instance.new(options.new, options.content, function(err, gist) {
                ...
                logger.defaultCallback(
                    err, null, logger.compileTemplate('{{gistLink}}', { options: options }));
