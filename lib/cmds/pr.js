...
PullRequest.prototype.run = function() {
    ...
    if (options.fetch) {
        ...
        if (options.all) {
            ...
            instance.fetch(instance.currentRepository, options.branch, options.pull, fetchType);
        }
    }
    ...
    if (options.submit) {
        instance.submit(
            ...
            options.branch, instance.currentBranch);
    }
};
    ...
    instance.getPullRequest_(repository, pullNumber, function(err, pull) {
        ...
        git.exec(
            ...
            function(err) {
                ...
                logWithPullNumber(pullNumber, 'fetch into branch ' + opt_branch);
                ...
                if (fetchType === PullRequest.FETCH_TYPE_REBASE_) {
                    ...
                    instance.checkoutPullRequestBranch_(headBranch, opt_branch, pullNumber);
                }
            }
        );
    });
...
PullRequest.prototype.fetchWithRebase_ = function(opt_branch, pullNumber) {
    ...
    git.exec('rebase', [ opt_branch ], function(err) {
        ...
        logWithPullNumber(
            pullNumber,
            'fetch and rebased into ' + instance.currentBranch);
    });
};
...
PullRequest.prototype.getPullRequest_ = function(repository, pullNumber, opt_callback) {
    var config,
        payload;
...
PullRequest.prototype.submit = function(
    submitUser, repository, headBranch, opt_toBranch, title) {
    ...
    payload = {
        ...
        title: title,
        body: '',
        base: opt_toBranch || config.defaultbranch,
    ...
    operations = [
        ...
        function(callback) {
            base.logger.info(
                'sending ' + user + ':' + headBranch + ' to ' + submitUser + ':' + opt_toBranch);
