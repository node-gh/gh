...
PullRequest.prototype.run = function() {
    ...
    if (!options.fetch && options.merge) {
        instance.merge(
            options.pull, options.branch || config.defaultbranch, false);
    }
    ...
    if (!options.fetch && options.rebase) {
        instance.merge(
            options.pull, options.branch || config.defaultbranch, true);
    }
...
PullRequest.prototype.checkout_ = function(opt_branch, opt_newBranch, opt_callback) {
    var args;

    opt_branch = opt_branch || base.getGlobalConfig().defaultbranch;
    ...
    git.exec('checkout', args, function(err, data) {
        if (!err) {
            logger.info('checkout ' + (opt_newBranch || branch));
        }
...
PullRequest.prototype.close = function(repository, number) {
    ...
    pullBranch = instance.currentBranch;
        ...
        instance.updatePullRequest_(
            ...
            function(err) {
                if (!err) {
                    ...
                    instance.checkout_(null, null, function(err) {
                        if (!err) {
                            git.exec('branch', [ '-D', pullBranch ], function(err) {
                                if (!err) {
                                    base.logger.info('delete branch ' + pullBranch);
                                }
                            });
                        }
                    });
        ...
        git.exec(
            ...
            function(err) {
                ...
                if (fetchType === PullRequest.FETCH_TYPE_REBASE_) {
                    git.merge(opt_branch, true, true, function(err, data) {
                        console.log(data);
                        logWithPullNumber(
                    ...
                    git.merge(opt_branch, false, true, function(err, data) {
                        console.log(data);
                        logWithPullNumber(
    ...
    operations = [
        function(callback) {
            instance.checkout_(branch, null, function(err) {
                if (err) {
                    logger.error('unable to checkout ' + branch);
                }
                callback(err);
            });
        },
        function(callback) {
            ...
            git.merge(instance.currentBranch, opt_rebase, false, function(err, data) {
                console.log(data);
                callback(err);
            });
        }
    ...
    operations = [
        ...
        function(callback) {
            ...
            base.github.pullRequests.create(payload, function(err, pull) {
                if (err) {
                    logger.error('cannot send pull request');
                }
...
PullRequest.prototype.updatePullRequest_ = function(
    ...
    base.github.pullRequests.update(payload, function(err, pull) {
        opt_callback = opt_callback && opt_callback(err, pull);

        if (err) {
            logger.error('unable to update pull request');
        }
    });
};
