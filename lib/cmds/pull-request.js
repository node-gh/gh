...
PullRequest.prototype.run = function() {
    ...
    if (options.list) {
        if (options.all) {
            if (options.me) {
                logger.logTemplate('{{prefix}} [info] Listing all {{options.state}} pull requests sent by {{greenBright options.loggedUser}}', {
                    options: options
                });
            }
            else {
                logger.logTemplate('{{prefix}} [info] Listing all {{options.state}} pull requests for {{greenBright options.user}}', {
                    options: options
                });
            }
    ...
    operations = [
        function(callback) {
            base.github.pullRequests.getAll(payload, function(err, data) {
                if (!err) {
                    if (options.me) {
                        pulls = data.filter(function(pull) {
