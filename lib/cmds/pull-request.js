...
PullRequest.prototype.run = function() {
    ...
    if (options.close) {
        hooks.invoke('pull-request.close', instance, function(afterHooksCallback) {
            logger.logTemplate(
                '[info] Closing pull request {{greenBright "#" options.number}}', {
                    options: options
                });
    ...
    if (options.comment) {
        logger.logTemplate(
            '[info] Adding comment on pull request {{greenBright "#" options.number}}', {
                options: options
            });
    ...
    if (options.fwd) {
        hooks.invoke('pull-request.fwd', instance, function(afterHooksCallback) {
            logger.logTemplate(
                '[info] Forwarding pull request {{greenBright "#" options.number}} to {{{magentaBright "@" options.fwd}}}', {
                    options: options
                });
            ...
            instance.forward(function(err, pull) {
                ...
                logger.compileTemplate('{{forwardedLink}}', {
                    options: options
                });
    ...
    if (options.list) {
        ...
        if (options.all) {
            logger.logTemplate(
                '[info] Listing all {{options.state}} pull requests for {{greenBright options.user}}', {
                    options: options
                });
            ...
            if (options.me) {
                logger.logTemplate(
                    '[info] Listing {{options.state}} pull requests sent by {{greenBright options.loggedUser}} on {{greenBright options.user "/" options.repo}}', {
                        options: options
                    });
            }
            else {
                logger.logTemplate(
                    '[info] Listing {{options.state}} pull requests on {{greenBright options.user "/" options.repo}}', {
                        options: options
                    });
            }
    ...
    if (options.open) {
        hooks.invoke('pull-request.open', instance, function(afterHooksCallback) {
            logger.logTemplate(
                '[info] Opening pull request {{greenBright "#" options.number}}', {
                    options: options
                });
    ...
    if (options.submit) {
        hooks.invoke('pull-request.submit', instance, function(afterHooksCallback) {
            logger.logTemplate(
                '[info] Submitting pull request to {{{magentaBright "@" options.submit}}}', {
                    options: options
                });

            instance.submit(options.submit, function(err, pull) {
                if (err) {
                    logger.error('Can\'t submit pull request to ' + options.submit + '.');
                    return;
                }
                ...
                logger.compileTemplate('{{submittedLink}}', {
                    options: options
                });
...
PullRequest.prototype.fetch = function(opt_type, opt_callback) {
    ...
    instance.getPullRequest_(function(err, pull) {
        ...
        if (opt_type === PullRequest.FETCH_TYPE_REBASE) {
            git.merge(options.pullBranch, true, true);
        }
        else if (opt_type === PullRequest.FETCH_TYPE_MERGE) {
            git.merge(options.pullBranch, false, true);
        }
        else if (opt_type === PullRequest.FETCH_TYPE_CHECKOUT) {
            git.checkout(options.pullBranch);
        }
...
PullRequest.prototype.merge = function(opt_callback) {
    ...
    operations = [
        ...
        function(callback) {
            git.checkout(options.branch);
            git.merge(options.pullBranch, options.rebase, false);
            git.push(config.default_remote, options.branch);
            git.deleteCurrent(options.pullBranch);
            callback();
        }
    ];
