    ...
    operations = [
        ...
        function(callback) {
            if (options.pullBranch) {
                git.spawnGit('branch', ['-D', options.pullBranch]);
            }
...
PullRequest.prototype.fetch = function(opt_type, opt_callback) {
    ...
    operations = [
        ...
        function(callback) {
            git.spawnGit('fetch', [repoUrl, headBranch + ':' + options.pullBranch], function(status,
                err) {
                callback(err);
            });
        },
...
PullRequest.prototype.merge = function(opt_callback) {
    ...
    operations = [
        ...
        function(callback) {
            git.spawnGit('push', [config.default_remote, options.branch], function(err) {
                if (err) {
        ...
        function(callback) {
            git.spawnGit('branch', ['-D', options.pullBranch], callback);
        }
    ];
...
PullRequest.prototype.submit = function(user, opt_callback) {
    ...
    operations = [
        ...
        function(callback) {
            git.spawnGit('push', [config.default_remote, pullBranch], function(err) {
                if (err) {
                    err = 'Unable to push local branch.';
                }
                callback(err);
