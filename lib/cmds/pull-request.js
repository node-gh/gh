...
PullRequest.prototype.printPullsInfoTable_ = function (pulls) {
    ...
    function generateTable() {
        ...
        pulls.forEach(pull => {
            const { createdTime, number, prInfo, user, status } = getColorizedFields(pull, isCompact ? getTotalWidth() : tableWidths[1]);
            const body = [
    ...
    function getColorizedFields(pull, length) {
        ...
        const status = pull.combinedStatus === 'success' ? logger.colors.green('✓') : logger.colors.red('✗');
