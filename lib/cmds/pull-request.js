    ...
    if (options.fetch) {
        ...
        hooks.invoke('pull-request.merge', instance, function(afterHooksCallback) {
            ...
            instance.merge(function(err, pull) {
                git.getLastCommitSHA(function (err, data) {
                    options.currentSHA = data;
...
PullRequest.prototype.browser = function(user, repo, number) {
    open('https://github.com/' + user + '/' + repo + '/pull/' + number);
};
...
PullRequest.prototype.getBranchNameFromPullNumber_ = function(number) {
    if (number) {
        return base.getGlobalConfig().pull_branch_name_prefix + number;
    }
};
    ...
    operations = [
        ...
        function(callback) {
            ...
            if (options.issue) {
                ...
                base.github.pullRequests.createFromIssue(params, callback);
            }
            else {
                params.title = options.title || title;
                params.body = options.description;
                base.github.pullRequests.create(params, callback);
            }
        }
    ];
