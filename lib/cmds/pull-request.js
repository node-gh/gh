...
PullRequest.prototype.run = function() {
    ...
    if (options.list) {
        ...
        if (options.all) {
            if(options.org) {
                if(options.me) {
                    loggerFormat = '{{prefix}} [info] Listing all {{options.state}} pull requests for {{greenBright options.org}} sent by {{greenBright options.loggedUser }}';
                } else {
                    loggerFormat = '{{prefix}} [info] Listing all {{options.state}} pull requests for {{greenBright options.user}}';
                }
                ...
                instance.listFromAllOrgRepositories(function(err) {
