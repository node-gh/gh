...
function execSync(args) {
    var exec = spawnSync(git_command, args);

    return {
        stdout: new Buffer(exec.stdout).toString(),
        stderr: new Buffer(exec.stderr).toString(),
        status: exec.status
    };
}
...
exports.findRoot = function() {
    return execSync(['rev-parse', '--show-toplevel']).stdout.trim();
};
...
exports.getConfig = function(key) {
    var git = execSync(['config', '--get', key]);
...
exports.getCurrentBranch = function() {
    var git = execSync(['symbolic-ref', 'HEAD']);

    if (git.status !== 0) {
        return;
    }

    return git.stdout.trim().substring(git.stdout.lastIndexOf('/') + 1);
};
...
exports.getLastCommitSHA = function() {
    var git = execSync(['rev-parse', '--short', 'HEAD']);
